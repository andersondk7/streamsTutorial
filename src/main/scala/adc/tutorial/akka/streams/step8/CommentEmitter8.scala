...
/**
  ...
  */
class CommentEmitter8(max: Int, preFetch: Int, bufferSize: Int) {
  ...
  def executeWith(f: (Comment) => Unit)(implicit system: ActorSystem): Future[Done] = {
    ...
    futureQueue map { queue => system.actorOf(SourceActor8.props(max=max, preFetch=preFetch, queue=queue)) }
    source.runForeach(f)
  }
