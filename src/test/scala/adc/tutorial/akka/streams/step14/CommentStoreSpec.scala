...
class CommentStoreSpec extends FunSpec with Matchers with BeforeAndAfterAll {
  ...
  describe("A CommentStore") {
    it("should retrieve no comments when empty") {
      ...
      val result: NextComment = Await.result( (commentStore ? Next).mapTo[NextComment], delay)
    ...
    it("should read fewer than it holds") {
      ...
      val total = 10
      val firstBatch = 3
      (1 to total).foreach(i => commentStore ! Insert(Comment(i, i, "name", "email", "body")))
      Thread.sleep(20) // a hack to let the requests all process
      val initialSize = Await.result( (commentStore ? Count).mapTo[CommentCount].map(_.size), delay)
      initialSize shouldBe total
      val first: Seq[Int] = (1 to firstBatch).map( i => {
        Await.result( (commentStore ? Next).mapTo[NextComment], delay)
